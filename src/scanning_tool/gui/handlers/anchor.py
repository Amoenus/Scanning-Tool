...
def _handle_toggle_auto_alignment(
    ...
    ctx.config.auto_alignment.enabled = enabled
    ctx.scan_state.last_alignment_info.enabled = enabled
    ctx.scan_state.notify_alignment_info_listeners()
    status_updated.send(
        None,
        message=(
...
def _handle_toggle_anchor_overlay(
    ...
    ctx.overlay_state.anchor_overlay_visible = visible
    if visible:
        show_anchor_overlay(ctx.overlay_state, ctx.config.anchor_template)
        status_updated.send(None, message="Anchor overlay shown.")
    else:
        hide_anchor_overlay(ctx.overlay_state)
...
def _handle_update_alignment_poll_interval(
    ...
    ctx.config.alignment_poll_interval_ms = int(
        payload.get("value", ctx.config.alignment_poll_interval_ms),
    )
    status_updated.send(
        None,
        message=f"Alignment interval set to {ctx.config.alignment_poll_interval_ms} ms",
    )
...
def _handle_update_anchor_threshold(
    ...
    threshold = float(payload.get("value", ctx.config.anchor_threshold))
    ctx.config.anchor_threshold = max(0.1, min(0.99, threshold))
    if ctx.scan_state.anchor_tracker is not None:
        ctx.scan_state.anchor_tracker.set_threshold(ctx.config.anchor_threshold)
    status_updated.send(
        None,
        message=f"Anchor detection threshold set to {ctx.config.anchor_threshold:.2f}",
    )
...
def _handle_update_anchor_region(
    ...
    region = ctx.config.anchor_template
    ...
    update_anchor_overlay_region(ctx.overlay_state)
...
def _handle_update_anchor_offset(
    ...
    offset = ctx.config.anchor_offset
...
def _handle_reload_anchor_templates(
    ...
    ensure_anchor_directory(ctx.config.anchor_template_dir)
    if ctx.scan_state.anchor_tracker is None:
        ctx.scan_state.anchor_tracker = AnchorRegionTracker(
            ctx.config.anchor_template_dir,
            ScreenCaptureProvider(),
            ctx.config.anchor_threshold,
        )
    count = ctx.scan_state.anchor_tracker.set_directory(ctx.config.anchor_template_dir)
    status_updated.send(
        None,
        message=f"Loaded {count} anchor template(s) from {ctx.config.anchor_template_dir}.",
    )
...
def _handle_manual_realign(
    ...
    if _run_manual_realign(ctx.config, ctx.scan_state):
        info = ctx.scan_state.last_alignment_info
        status_updated.send(
            None,
            message=(
...
def _handle_open_anchor_directory(
    ...
    path = ctx.config.anchor_template_dir

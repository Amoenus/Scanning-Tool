...
def get_ollama_host() -> str:
    ...
    if manager.config.ollama_config.host:
        return manager.config.ollama_config.host
...
def set_configured_ollama_model(value: str) -> str:
    ...
    if sanitized:
        if sanitized != manager.config.ollama_config.model:
            manager.config.ollama_config.model = sanitized
            os.environ["OLLAMA_MODEL"] = sanitized
            manager.save_config()
    else:
        manager.config.ollama_config.model = ""

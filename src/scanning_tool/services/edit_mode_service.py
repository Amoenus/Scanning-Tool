...
class EditModeService(BaseService):
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        data = EditRegionPayload.model_validate((payload or {}) | kwargs)
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: EditRegionPayload) -> dict[str, int]:
        if region == ActiveRegion.CAPTURE or region == ActiveRegion.ANCHOR:
            ...
            left = int(payload.left if payload.left is not None else source.left) + payload.delta_left
            top = int(payload.top if payload.top is not None else source.top) + payload.delta_top
            width = int(payload.width if payload.width is not None else source.width) + payload.delta_width
            height = int(payload.height if payload.height is not None else source.height) + payload.delta_height
        ...
        if region == ActiveRegion.INFO:
            ...
            x = int(payload.x if payload.x is not None else info_source.x) + (
                payload.delta_x if payload.delta_x is not None and payload.delta_x != 0 else payload.delta_left
            )
            y = int(payload.y if payload.y is not None else info_source.y) + (
                payload.delta_y if payload.delta_y is not None and payload.delta_y != 0 else payload.delta_top
            )
            return {"x": x, "y": y}

...
class EditModeService(BaseService):
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        try:
            dto = RegionDragPayload.model_validate(self._normalize_payload(payload, kwargs))
    ...
    def _on_select_region(
        ...
        **kwargs: Any,
        ...
        try:
            dto = RegionSelectPayload.model_validate(self._normalize_payload(payload, kwargs))
        except pydantic.ValidationError:
            return
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: RegionDragPayload) -> dict[str, int]:
        if region == ActiveRegion.CAPTURE or region == ActiveRegion.ANCHOR:
            source = manager.config.capture_region if region == ActiveRegion.CAPTURE else manager.config.anchor_template # type: ignore
            ...
            height = (payload.height if payload.height is not None else int(source.height)) + (
                payload.delta_height or 0
            )
        ...
        if region == ActiveRegion.INFO:
            ...
            x = (payload.x if payload.x is not None else int(source_info.x)) + (
                payload.delta_x if payload.delta_x is not None else (payload.delta_left or 0)
            )
            y = (payload.y if payload.y is not None else int(source_info.y)) + (
                payload.delta_y if payload.delta_y is not None else (payload.delta_top or 0)
            )
            return {"x": x, "y": y}

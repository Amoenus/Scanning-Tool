...
class EditModeService(BaseService):
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        try:
            dto = RegionDragPayload.model_validate(combined_payload)
    ...
    def _on_select_region(
        ...
        **kwargs: Any,
        ...
        try:
            dto = RegionSelectPayload.model_validate(combined_payload)
        except pydantic.ValidationError:
            return
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: RegionDragPayload) -> dict[str, int]:
        if region == ActiveRegion.CAPTURE or region == ActiveRegion.ANCHOR:
            source_capture = manager.config.capture_region if region == ActiveRegion.CAPTURE else manager.config.anchor_template
            left = (payload.left if payload.left is not None else source_capture.left) + (payload.delta_left if payload.delta_left is not None else 0)
            top = (payload.top if payload.top is not None else source_capture.top) + (payload.delta_top if payload.delta_top is not None else 0)
            width = (payload.width if payload.width is not None else source_capture.width) + (payload.delta_width if payload.delta_width is not None else 0)
            height = (payload.height if payload.height is not None else source_capture.height) + (payload.delta_height if payload.delta_height is not None else 0)
            return {"left": int(left), "top": int(top), "width": max(1, int(width)), "height": max(1, int(height))}

        if region == ActiveRegion.INFO:
            source_info = manager.config.overlay_config.info_offset
            delta_x = payload.delta_x if payload.delta_x is not None else (payload.delta_left if payload.delta_left is not None else 0)
            delta_y = payload.delta_y if payload.delta_y is not None else (payload.delta_top if payload.delta_top is not None else 0)
            x = (payload.x if payload.x is not None else source_info.x) + delta_x
            y = (payload.y if payload.y is not None else source_info.y) + delta_y

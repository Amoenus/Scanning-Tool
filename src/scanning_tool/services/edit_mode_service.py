...
class EditModeService(BaseService):
    def __init__(self) -> None:
        ...
        self._pending_region_payloads: dict[ActiveRegion, EditModePayload] = {}
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        dto = EditModePayload.model_validate(self._normalize_payload(payload, kwargs))
    ...
    def _on_commit_region(self, sender: object, **kwargs: Any) -> None:
        ...
        final_payload = self._pending_region_payloads.get(
            ...
            self._payload_for_region(self._state.active_region, EditModePayload()),
        )
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: EditModePayload) -> EditModePayload:
        if region == ActiveRegion.CAPTURE or region == ActiveRegion.ANCHOR:
            ...
            left = (payload.left if payload.left is not None else source.left) + (payload.delta_left or 0)
            top = (payload.top if payload.top is not None else source.top) + (payload.delta_top or 0)
            width = (payload.width if payload.width is not None else source.width) + (payload.delta_width or 0)
            height = (payload.height if payload.height is not None else source.height) + (payload.delta_height or 0)
            return EditModePayload(left=left, top=top, width=max(1, width), height=max(1, height))

        if region == ActiveRegion.INFO:
            source = manager.config.overlay_config.info_offset
            x = (payload.x if payload.x is not None else source.x) + (payload.delta_x or payload.delta_left or 0)
            y = (payload.y if payload.y is not None else source.y) + (payload.delta_y or payload.delta_top or 0)
            return EditModePayload(x=x, y=y)

        return EditModePayload()

    def _capture_original_region_payloads(self) -> dict[ActiveRegion, EditModePayload]:
    ...
    def _publish_region_config_action(
        ...
        if signal is not None:
            signal.send(self, payload=payload.model_dump(exclude_none=True))

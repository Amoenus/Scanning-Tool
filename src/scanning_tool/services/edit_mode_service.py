...
class EditModeService(BaseService):
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        data = EditModePayload.model_validate({**normalized_payload, **kwargs})
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: EditModePayload) -> dict[str, int]:
        ...
        if region == ActiveRegion.INFO:
            ...
            delta_x = int(payload.delta_x if payload.delta_x is not None else (payload.delta_left or 0))
            delta_y = int(payload.delta_y if payload.delta_y is not None else (payload.delta_top or 0))
            x = int(payload.x if payload.x is not None else info_source.x) + delta_x
            y = int(payload.y if payload.y is not None else info_source.y) + delta_y
            return {"x": x, "y": y}

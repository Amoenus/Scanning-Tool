...
class EditModeService(BaseService):
    ...
    def _on_drag_region(
        ...
        **kwargs: Any,
        ...
        try:
            data = EditModePayload.model_validate(normalized)
        except Exception:
            return
    ...
    def _payload_for_region(self, region: ActiveRegion, payload: EditModePayload) -> dict[str, int]:
        if region == ActiveRegion.CAPTURE or region == ActiveRegion.ANCHOR:
            ...
            p_left = payload.left if payload.left is not None else int(source.left)
            p_top = payload.top if payload.top is not None else int(source.top)
            p_width = payload.width if payload.width is not None else int(source.width)
            p_height = payload.height if payload.height is not None else int(source.height)

            left = p_left + (payload.delta_left or 0)
            top = p_top + (payload.delta_top or 0)
            width = p_width + (payload.delta_width or 0)
            height = p_height + (payload.delta_height or 0)
        ...
        if region == ActiveRegion.INFO:
            ...
            p_x = payload.x if payload.x is not None else int(info_source.x)
            p_y = payload.y if payload.y is not None else int(info_source.y)

            x = p_x + (payload.delta_x if payload.delta_x is not None else (payload.delta_left or 0))
            y = p_y + (payload.delta_y if payload.delta_y is not None else (payload.delta_top or 0))
            return {"x": x, "y": y}
